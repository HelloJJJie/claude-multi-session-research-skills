...
- **"Full depth" is not license to merge separate atomic units into one note.** A concept's result/definition and a substantial derivation, proof, or comparison built on it are different atomic units even when they're "about the same concept" — split the derivation into its own note (named for what it establishes, e.g. "X 的证明", "X vs Y 对照") and link both directions, rather than appending it to the note that states the result. A derivation that *is* the concept in one short step (no sub-lemmas of its own) can stay inline — the line is whether the material could stand alone as something worth linking *to*, not length for its own sake. Evidence: a note stating a formula's result grew a full multi-step derivation and a comparison table under the "carry full depth" instruction above, becoming one long file, until the user asked for the result and its derivation to be separate notes joined by a link.
